...
class Template:
    ...
    substitutions = {
        TemplateConstants.DOLLAR_CURLY_BRACE: TemplateConstants.SubPair(
            re.compile(r"\${.*?}+"), slice(2, -1)
        ),
        TemplateConstants.DOLLAR_PARENTHESES: TemplateConstants.SubPair(
            re.compile(r"\$\(.*?\)+"), slice(2, -1)
        ),
        TemplateConstants.DOUBLE_CURLY_BRACES: TemplateConstants.SubPair(
            re.compile(r"{{.*?}}+"), slice(2, -2)
        ),
        TemplateConstants.AT_SQUARE_BRACES: TemplateConstants.SubPair(
            re.compile(r"@\[.*?\]+"), slice(2, -1)
        ),
                ...
                if v is not None:
                    if not is_single_type_or_string(v):
                        if len(variable_names) == 1:
                            ...
                            if (
                                variable_to_substitute.replace(
                                    variable_names[0][pair.slice], ""
                                )
                                == var_type
                            ):
    ...
    def substitute_structure(cls, structure_to_substitute, var_type: str, get_value):
        ...
        if isinstance(structure_to_substitute, dict):
            ...
            structure_to_substitute = cls.substitute_string(
                structure_to_substitute, var_type, get_value
            )
        return structure_to_substitute

    @classmethod
    def substitute_structure_from_environment(cls, structure_to_substitute):
        return cls.substitute_structure(
            structure_to_substitute,
            TemplateConstants.DOLLAR_CURLY_BRACE,
            os.environ.get,
        )

    @classmethod
    def substitute_with_dependencies(
        cls, dictionary, keys, var_type: str, shallow_precedence=True, excluded=()
    ):
        """
        ...
        def build(structure, variables):
            if isinstance(structure, dict):
                for k, i in structure.items():
                    if (
                        (k not in variables)
                        or (k in variables and not shallow_precedence)
