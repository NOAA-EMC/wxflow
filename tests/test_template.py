...
def test_substitute_string_from_dict():
    ...
    dictionary = {"greeting": "Hello", "the_world": "the world"}
    final = "Hello to the world"
    assert (
        Template.substitute_structure(
            template, TemplateConstants.DOLLAR_CURLY_BRACE, dictionary.get
        )
        == final
    )
...
def test_substitute_string_from_dict_paren():
    ...
    assert (
        Template.substitute_structure(
            template, TemplateConstants.DOLLAR_PARENTHESES, dictionary.get
        )
        == final
    )
...
def test_substitute_string_from_dict_double_curly():
    ...
    assert (
        Template.substitute_structure(
            template, TemplateConstants.DOUBLE_CURLY_BRACES, dictionary.get
        )
        == final
    )
...
def test_substitute_string_from_dict_at_square():
    ...
    assert (
        Template.substitute_structure(
            template, TemplateConstants.AT_SQUARE_BRACES, dictionary.get
        )
        == final
    )
...
def test_substitute_string_from_dict_at_carrots():
    ...
    assert (
        Template.substitute_structure(
            template, TemplateConstants.AT_ANGLE_BRACKETS, dictionary.get
        )
        == final
    )
...
def test_substitute_with_dependencies():
    ...
    output = {
        ...
        "greeting": "hello world",
        "host": {
            "config": "/home/user/hosts",
            "config_file": "/home/user/config/config.yaml/xenon.config.yaml",
            "name": "xenon",
            "proxy2": {
                "config": "/home/user/xenon.hello world.yaml",
                "list": [["/home/user/xenon", "toto.xenon.hello world"], "config.yaml"],
            },
        },
        "root": "/home/user",
        "world": "world",
    ...
    assert (
        Template.substitute_with_dependencies(
            input, input, TemplateConstants.DOLLAR_PARENTHESES
        )
        == output
    )
