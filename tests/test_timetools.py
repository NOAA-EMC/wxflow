...
def test_to_timedelta():
    ...
    assert to_timedelta("5 days, 12:30:15") == timedelta(
        days=5, hours=12, minutes=30, seconds=15
    )
...
def test_strptime():
    assert strptime(current_date.strftime("%Y%m%d"), "%Y%m%d") == datetime.strptime(
...
def test_add_to_timedelta():
    assert add_to_timedelta(timedelta(days=1), timedelta(hours=3)) == timedelta(
        days=1, hours=3
    )
    assert add_to_timedelta(
        timedelta(hours=5, minutes=30), timedelta(minutes=15)
    ) == timedelta(hours=5, minutes=45)
    assert add_to_timedelta(
        timedelta(seconds=45), timedelta(milliseconds=500)
